...
name1="Nature-Brain.png"
name2="nature-1.png"
#import image from data folder
ad=os.getcwd()
ad=ad+'\data\\'
I=imageio.imread(ad+name1)
...
d=np.zeros((K,s[0]*s[1]))
for i in np.arange(K):
        d[i,:]=np.sum((np.tile(np.reshape(mu[:,i],(s[2],1),order='C'),(1,s[0]*s[1]))-E)**2,axis=0)
r=np.equal(d,np.tile(np.amin(d,axis=0),(K,1)))*1
...
I_new=np.dstack((R,G,B))/255

fig=plt.figure(figsize=[10,10],dpi=300)
plt.imshow(I_new)
plt.axis('off')
# fig.savefig('p3_e2k4.svg',format='svg')
# fig.savefig('p3_e2k4.tif')
